...
- [x] **LTD badge is illegible / unexplained** — RESOLVED 2026-08-23 on `feat/player-card-talent-v1` across all 3 card variants (Season/Talent/History): the old gray "LTD"/`limited sample` label is replaced everywhere by the locked `LTD SAMPLE` tinted-red pill grammar (render-tested before/after on Kastelic 2021-22). Was: gray label for TOI-gated prior seasons (< 500 5v5 min, `war_status: "limited_toi"`) that blended into the card background. See receipt `docs/plans/player-card-talent-v1-migration.md`.
  - [x] **Talent+Shotmap hero** — resolved on `feat/player-card-talent-v1` (2026-08-23): `LTD SAMPLE` / `EARLY SAMPLE · N GP` tinted-red chips beneath the hero values (locked chip grammar), render-tested. See `docs/plans/player-card-talent-v1-migration.md`.
  - [x] **Masthead Talent tile** — resolved on `feat/player-card-talent-v1` (2026-08-23): `LTD SAMPLE` chip (Talent-card grammar) absolutely positioned bottom-center inside the Talent tile; shared `.hero-tile` `min-height` keeps all three tiles equal peers with one baseline (chip overlays whitespace, no tile growth, no fake badge row). Render-verified full vs limited. See receipt.
  - [x] **Season card variant** — resolved 2026-08-23: the HGB WAR hero tile's gray `limited sample` note is now the `LTD SAMPLE` tinted-red pill (provisional note untouched). Render before/after verified.
...
- [ ] **HGBTable CSV export ignores `exportText` (uses `accessor`)** — surfaced Jul 2026 during the season-projections mock. `exportCSV` reads `column.accessor` while `handleExportPng` uses `exportText`, so for any column where the two differ, **CSV and PNG exports disagree** (e.g. a column that renders an abbr on-page but should export a full name: CSV gets the abbr, PNG gets the exportText string). Workaround used on projections = set `accessor` to the export value too. Real fix: make `exportCSV` honor `exportText` (fall back to accessor) so both paths match. Shared HGBTable change — audit existing tables' exportText usage before changing, since some may rely on the current CSV-uses-accessor behavior.
- [x] **HGBTable internal filter chips + export buttons → match the /stats look** — DONE Jul 8 2026. CORRECTION to the original framing: HGBTable's built-in `filters` `ChipGroup` is NOT used by every stats table — the /stats pages (skaters/goalies/teams/lines/impact) use `FilterPrimitives.FilterChip` (already segmented Barlow-Semi-Condensed). HGBTable's built-in chips were used only by **ProjectionsTable** (ALL/EAST/WEST). Fixed by making HGBTable's built-in `ChipGroup` **reuse `FilterPrimitives.FilterChip/FilterChipGroup`** (one chip style, no third variant), and aligning HGBTable's built-in CSV/PNG export buttons to the same SEMI/white-fill style skaters uses. Deployed. NOTE: this leaves THREE chip stylings still coexisting site-wide (`FilterPrimitives`=stats-table filters, `ChipGroup.astro`=page-nav chrome, now unified for the HGBTable path) — full site-wide chip unification (FilterPrimitives vs ChipGroup.astro) is a separate larger effort, not pursued.
...
- [x] **Player Talent card — Talent-v1 migration + hero redesign** (2026-08-23, `feat/player-card-talent-v1`, FROZEN/approved, NOT deployed): migrated displayed Talent percentiles `hgb_rating_percentile` → verified `hgb_talent_v1_percentile` (schema fields added; peer-table sort → raw `hgb_talent_v1` with recalibrated bands); redesigned the Talent+Shotmap hero into a 63/37 Talent | `CURRENT ROLE` (TOI/G) split; empty-WAR strip rewritten in the card's condensed-uppercase metadata voice (`NO PRIOR WAR ON RECORD`); removed dead `drawRatingCardHorizontal`/`drawRatingCardPortrait`; masthead `LTD SAMPLE` chip (overlaid, fixed-size peer tiles) + History-card per-season `TOI/G` column added 2026-08-23. 155/155 tests, zero new typecheck errors, all hero states + both deferred changes render-tested. Full receipt: `docs/plans/player-card-talent-v1-migration.md`.
