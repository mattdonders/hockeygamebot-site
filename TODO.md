...
- [ ] **HGBTable internal filter chips → canonical ChipGroup spec** (low priority) — HGBTable's built-in `filters={[{type:'chips'}]}` renderer (`HGBTable.tsx:240`, internal React `ChipGroup`) is off-spec vs `ChipGroup.astro`/CHIPS.md: uses `--mono` 10px separate-bordered buttons, no hover; canonical is `--body` Barlow 700/11px, adjoining segmented, ink-06 hover (active state already matches). Shared by EVERY stats table (skaters/goalies/teams/lines/impact/projections), so a fix brings them all to brand compliance at once — but the blast radius means it needs a review against a couple real pages before merge. Surfaced during the season-projections mock (Jul 2026); not hacked page-local to avoid desyncing the fleet.
